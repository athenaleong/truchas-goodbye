...
app.get("/getTag", async function(req, res) {
  let id = req.query.id;
  tagCollection.findOne({ _id: id }).then((tag) => {
    res.send(tag);
  });
});
...
app.get("/getImage", async function(req, res) {
  ...
  const getUrl = (id) =>
    async function() {
      const filePromise = imageFileCollection.findOne({
        _id: new ObjectID(id),
      });
      const chunkPromise = imageChunkCollection
  ...
  Promise.all(idArray.map((id) => getUrl(id)())).then((values) => {
    res.send(values);
  });
});
